...
Expected file structure:
    ...
    test_images_results/
        img1_pred.npy           <- predicted by run_pipeline.py (in crop space)
        img1_bbox.npy           <- bbox saved by run_pipeline.py (x1, y1, x2, y2)
...
GT_DIR         = '../raw_dataset/preprocessed/test_images'
PRED_DIR       = '../raw_dataset/preprocessed/cropped_test_images_results'
...
def print_report(pairs, total_gt, yolo_detected):
    ...
    # PCK — % and image counts
    print(f"\n  {'Threshold':<12} {'% Correct':>10}   {'Images all-KP correct':>22}")
    print(f"  {'-'*48}")
    for tau in PCK_THRESHOLDS:
        pck_pct = pck(all_dist_flat, tau)

        # Count images where ALL keypoints are within threshold
        imgs_all_correct = sum(
            1 for d in all_distances if np.all(d < tau)
        )
