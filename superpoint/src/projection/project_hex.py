"""
Script 2: Project hex centers from ideal template → test image space
...
def project_hex_centers(hex_centers, H):
    ...
    # perspectiveTransform expects shape (M, 1, 2)
...
def visualize_hex(test_img, projected_hex, template_kps, test_kps, mask, save_path):
    ...
    # Draw keypoints used for homography
    ...
    # Draw projected hex centers
    for hex_id, (x, y) in projected_hex.items():
        x, y = int(x), int(y)
        cv2.circle(canvas, (x, y), 5, (0, 165, 255), -1)   # orange dot
        cv2.circle(canvas, (x, y), 5, (0, 0, 0), 1)         # black border
        cv2.putText(canvas, hex_id, (x + 6, y - 6),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 165, 255), 1, cv2.LINE_AA)
        # cv2.putText(canvas, hex_id, (x-6, y),
        #         cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 255), 1, cv2.LINE_AA)  # Red color
    # Legend
    h = canvas.shape[0]
